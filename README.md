...
- 连通域形状分型：细长带=墙 / 小团=柱 / 中椭圆团(3~8m×2m 长宽比<1.4)=车→剔除；
- 输出 `*_梯度墙柱.png`（红=墙, 橙=柱, 灰=车/其余）。

阶段三·坡道并入（`code/outline.py` v0.28，`detect_ramp`）：
- 坡道=**从主体轮廓外沿伸出的窄长占位带**（密度梯度思想：点从主体连续延伸出去）。
  0.5m 栅格占位 → 与正交主体轮廓做差集 → 外 15m 带伸出结构连通域 → 细长校验
  → PCA 定向矩形（斜入口保留斜向）→ **逐个矩形护栏**（红点 d² 不劣化才并入）
  → union 成**一个闭合体**。
- `ramps: []` 表示无入口（结果与 v0.27 一致）；`ramps` 为入口定向矩形四角坐标。
- `RAMP_*` 参数见 `code/outline.py` 顶部（栅格 0.5m / 外带 15m / 矩形 2~14m 宽等）。
